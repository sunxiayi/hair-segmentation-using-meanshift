# USAGE
# python facial_landmarks.py --shape-predictor shape_predictor_68_face_landmarks.dat --image images/example_01.jpg 

# import the necessary packages
from imutils import face_utils
import numpy as np
import argparse
import imutils
import dlib
import cv2

def parse_arguments():
	# construct the argument parser and parse the arguments
	ap = argparse.ArgumentParser()
	ap.add_argument("-p", "--shape-predictor", required=True,
		help="path to facial landmark predictor")
	ap.add_argument("-i", "--image", required=True,
		help="path to input image")
	args = vars(ap.parse_args())
	return args

def head_detector(args, detector, predictor):
	# load the input image, resize it, and convert it to grayscale
	image = cv2.imread(args["image"])
	original_height = image.shape[0]
	original_width = image.shape[1]
	image = cv2.resize(image,(50,50),interpolation=cv2.INTER_CUBIC)
	
	#gray = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)

	# detect faces in the grayscale image
	# rect = detector(gray, 1)[0]

	# determine the facial landmarks for the face region, then
	# convert the facial landmark (x, y)-coordinates to a NumPy
	# array
	#shape = predictor(gray, rect)
	#shape = face_utils.shape_to_np(shape)

	# convert dlib's rectangle to a OpenCV-style bounding box
	# [i.e., (x, y, w, h)], then draw the face bounding box
	#(x, y, w, h) = face_utils.rect_to_bb(rect)

	# from face to head
	#x = max(0, x - w / 2)
	#y = max(0, y - 2*h / 3)
	#w = 2 * w
	#h = 3 * h
	#cv2.rectangle(image, (x, y), (x + w, y + h), (0, 255, 0), 2)

	# show the head number
	#cv2.putText(image, "Head #{}".format(i + 1), (x - 10, y - 10),
	#	cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)

	# loop over the (x, y)-coordinates for the facial landmarks
	# and draw them on the image
	#for (i, j) in shape:
	#	cv2.circle(image, (i, j), 1, (0, 0, 255), -1)

	#print(image.shape[0], image.shape[1])

	# show the output image with the face detections + facial landmarks
	# cv2.imshow("Output", image)

	#cv2.imwrite("head.png", image)
	
	
	all_points = []
	# print(x,y,w,h)

	'''
	for i in range(x, min(x + w, image.shape[0])):
		for j in range(y, min(y + h, image.shape[1])):
			all_points.append(np.array([i,j] + list(image[i,j])))
	'''

	for i in range(0, image.shape[0]):
		for j in range(0, image.shape[1]):
			all_points.append(np.array([i,j] + list(image[i,j]))) # revised

	print("a")
	return image, all_points, original_width, original_height

def get_points(args):
	#args = parse_arguments()
	# initialize dlib's face detector (HOG-based) and then create
	# the facial landmark predictor
	detector = dlib.get_frontal_face_detector()
	predictor = dlib.shape_predictor(args["shape_predictor"])

	# get all points in the head region
	return head_detector(args, detector, predictor)

#h = get_points()